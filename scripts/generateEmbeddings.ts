...
// corpus with the WRONG model the next time someone runs it without explicitly setting the env var.
...
// Rounds each vector component to 6 decimal places — cosine similarity is completely insensitive
// to precision this fine (embedding components are typically in [-1, 1], so 6 decimals is ~4
// orders of magnitude finer than any meaningful similarity difference), but full float64 precision
// as JSON.stringify prints it wastes real space at this scale. Found live (2026-09-17): switching
// the embedding model (nomic-embed-text 768-dim -> bge-m3 1024-dim, see localLlmClient.ts) pushed
// this file from ~91MB to 116.69MB — over GitHub's 100MB hard limit, rejecting the push outright.
// Rounding plus dropping the pretty-print indent below brings it down to ~50MB, with real headroom
// for the corpus to keep growing.
...
async function main() {
  ...
  for (let i = 0; i < BUILTIN_KNOWLEDGE.length; i++) {
    ...
    vectors[item.id] = { vector: roundVector(result.vector), textHash };
    embedded++;
