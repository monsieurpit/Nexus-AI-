...
function buildLlmKnowledgeInstruction(reasoningMode: AISettings['reasoningMode']): string {
  return (
    ...
    // A real engaged friend doesn't just answer and stop — they stay curious about the other
    // person. Calibrated the same way as buildFinalDirective's chaotic-overshare instruction
    // (occasional, not constant) rather than forced mechanically on every Nth reply the way
    // swearing/overshare are — a generic tacked-on question can't be topically relevant without
