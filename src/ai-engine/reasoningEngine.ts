...
async function llmGroundedOrFallback(
  ...
  const groundedPrompt = usePolish
    ...
    : `The context below is only a loose/uncertain match for the user's question — it may not fully cover what they're actually asking. Use it as a starting point and answer as helpfully and knowledgeably as you genuinely can, drawing on your own broader knowledge too, but be honest about what's uncertain instead of inventing specifics you don't actually know. If the question is so vague or underspecified that you'd have to GUESS what topic they even mean (not just "uncertain about a fact", but genuinely no real idea what they're asking about — e.g. "how do I get better" with zero indication at what) — don't pick a random topic and confidently run with it, and don't just make a generic remark ABOUT the question being vague either. Actually ask them directly, as a real question ending in a question mark, addressed straight at them — literally something like "better at what, exactly?" or "what are you trying to fix?" — the way a real person would respond to a friend who just said something too vague to act on. Only do this when the topic itself is genuinely unclear, not just when a detail within an otherwise-clear question is missing. Your style directives (swearing, tone) still fully apply here — don't drop them just because you're being informative.\n\nContext:\n${groundingContext}\n\nQuestion: ${prompt}`;
